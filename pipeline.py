...
def data_prepare(args, dataset):
    ...
    for task in range(n_task):
        ...
        try:
            subgraph, ids_per_cls, [train_ids, valid_ids, test_ids] = pickle.load(open(f'{args.data_path}/{str_int_tsk}/{args.dataset}_as{args.n_agents}_t{task}_p{args.par}.pkl', 'rb'))
        except:
            ...
            if args.inter_task_edges:#prepare
                ...
                subgraph, ids_per_cls_all, [train_ids, valid_ids, test_ids] = dataset.get_graph(tasks_to_retain=cls_retain, n_agents=args.n_agents, partition=args.par)
                ...
                subgraph, ids_per_cls, [train_ids, valid_ids, test_ids] = dataset.get_graph(tasks_to_retain=task_cls, n_agents=args.n_agents,partition=args.par)
                with open(f'{args.data_path}/no_inter_tsk_edge/{args.dataset}_as{args.n_agents}_t{task}_p{args.par}.pkl', 'wb') as f:
...
def pipeline_class_IL_no_inter_edge(args, valid=False):
    ...
    for task in range(args.n_tasks):
        pro_cls={}
        subgraph, ids_per_cls, [train_ids, valid_ids, test_ids] = pickle.load(open(f'{args.data_path}/no_inter_tsk_edge/{args.dataset}_as{args.n_agents}_t{task}_p{args.par}.pkl', 'rb'))
        ...
        for rnd in range(args.n_rnds):
            for a_id in range(args.n_agents):
                ...
                if valid:
                    for epoch in range(epochs):
                        principal_list[a_id], principal_list_as[a_id], similarity_as[a_id], local_train_sizes[a_id], local_train_sizes_cls[a_id] = lms[a_id].observe_il(subgraph, features, labels - n_agents_cls_per_task * task, task, train_ids[a_id], n_agents_cls_per_task, lms, graph_matrix[a_id], a_id, fea_syn_as_as[a_id], g_syn_as_as[a_id], labels_syn_as_as[a_id], rnd, args)
                ...
                if rnd == args.n_rnds - 1:
                    if valid:
                        ...
                        for aid in range(args.n_agents):
                            prototypes_task.append(lms[a_id].getprototype(g_syn_as_as[a_id][aid][task], fea_syn_as_as[a_id][aid][task]))
                        prototypes_[task] = torch.nn.functional.normalize(torch.cat(prototypes_task, dim=0)).mean(0)

                    acc_mean = []
                    for t in range(task+1):
                        subgraph, ids_per_cls, [train_ids, valid_ids_, test_ids_] = pickle.load(open(f'{args.data_path}/no_inter_tsk_edge/{args.dataset}_as{args.n_agents}_t{t}_p{args.par}.pkl', 'rb'))
                        subgraph = subgraph.to(device='cuda:{}'.format(args.gpu))
                        features, labels = subgraph.srcdata['feat'], subgraph.dstdata['label'].squeeze()
                        ...
                        acc = evaluatewp(output, labels - args.n_cls_task*t, test_ids, cls_balance=args.cls_balance, ids_per_cls=ids_per_cls_test)
            ...
            if rnd == 0 and valid:
                ...
                for a_id in range(args.n_agents):
                    ...
                    for cls in range(n_agents_cls_per_task):
                        index = list(set(((labels - n_agents_cls_per_task * task) == cls).nonzero().view(-1).tolist()) & set(train_ids[a_id]))
                        if len(index) > 0:
            ...
            if rnd<args.n_rnds - 1 and valid:
                ...
                graph_matrix_cls_cs = []
                for cls in range(n_agents_cls_per_task):
                    model_complementary_matrix_cls = cal_complementary_cls(args.n_agents, np.array(principal_list_as)[:,cls])
                    print('complementary***********************', cls)
                    print(model_complementary_matrix_cls)
                    model_difference_matrix_cls = cal_model_cosine_difference_cls(args.n_agents, np.array(similarity_as)[:,cls], np.array(topo_fea_cls_as)[:,cls])
                    print('difference ***********************', cls)
                    print(model_difference_matrix_cls)
                    ratio_cls_ = (np.array(np.array(local_train_sizes_cls)[:,cls].squeeze()) / np.sum(np.array(local_train_sizes_cls)[:,cls].squeeze())).tolist()
                    ...
                    graph_matrix_cls_ = optimizing_graph_matrix_neighbor(args.n_agents, graph_matrix_cls_,
                                                                    model_complementary_matrix_cls, model_difference_matrix_cls,
                ...
                for i in range(args.n_agents):
                    for j in range(args.n_agents):
                        ...
                        ratio_cls_orig = (np.array(local_train_sizes_cls[j]) / np.sum(local_train_sizes_cls[j])).tolist()
                        mij = [args.w_col*graph_matrix_cls_cs[cls][i][j]+ratio_cls_orig[cls] for cls in range(n_agents_cls_per_task)]
                        ...
                        fea_syn_as_as[i][j][task], g_syn_as_as[i][j][task], labels_syn_as_as[i][j][task] = lms[
                            ...
                            mij, labels - n_agents_cls_per_task * task, train_ids[j], features, n_agents_cls_per_task, args)
